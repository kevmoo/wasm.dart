...
void main() {
  test('can compile and run custom hello world example', () async {
    ...
    final examplesDir = currentDir.endsWith('wasm_tools')
        ? p.join(currentDir, '../../examples/hello_world_custom')
        : p.join(currentDir, 'examples/hello_world_custom');
    ...
    expect(process.stdout, contains('Substring: world'));
    expect(process.stdout, contains('Equals "Hello": yes, "world": no'));
    expect(
      process.stdout,
      contains('CompareTo "Hello": 0, "Abc": 1, "Zzz": -1'),
    );
    expect(process.stdout, contains('FromCharCodes: Hello'));
