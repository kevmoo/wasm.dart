...
final class WasmRegExp {
  ...
  final Map<WasmStringImplementation, int> namedGroupCaptureIndices;
...
WasmExternRef embedderRegexpCreateOrFailWithString(
  ...
  final Map<WasmStringImplementation, int> namedCaptureIndices = {};
  ...
  if (namedCount > 0) {
    ...
    for (int i = 0; i < namedCount; i++) {
      ...
      namedNames.add(name);
      namedCaptureIndices[name] = capIdx;
...
WasmExternRef? embedderRegexpMatchGetGroup(
  ...
  final sub = WasmStringImplementation.substring(match.input, start, end);
...
WasmExternRef? embedderRegexpMatchGetGroupByName(
  ...
  if (idx < 0 || idx >= match.namedGroupNames.length) {
    return WasmExternRef.nullRef;
  }
  final name = match.namedGroupNames[idx];
  final capIdx = match.namedGroupCaptureIndices[name]!;
...
WasmExternRef? embedderStringReplaceAllString(
  ...
  if (nLen == 0) {
    ...
    for (int i = 0; i < len; i++) {
      final char = WasmStringImplementation.substring(string, i, i + 1);
      result = WasmStringImplementation.concat(result, char);
      result = WasmStringImplementation.concat(result, replacement);
    }
    return WasmAnyRef.fromObject(result).externalize();
  }
  ...
  int start = 0;
  while (true) {
    final idx = WasmStringImplementation.indexOfString(string, needle, start);
    if (idx == -1) {
      final rest = WasmStringImplementation.substring(string, start, len);
      result = WasmStringImplementation.concat(result, rest);
      break;
    }
    final prefix = WasmStringImplementation.substring(string, start, idx);
    result = WasmStringImplementation.concat(result, prefix);
    result = WasmStringImplementation.concat(result, replacement);
    start = idx + nLen;
  }
  return WasmAnyRef.fromObject(result).externalize();
...
WasmExternRef? embedderStringReplaceAllRegExp(
  ...
  while (start <= len) {
    ...
    final prefix = WasmStringImplementation.substring(
      string,
      start,
      matchStart,
    );
    ...
    if (matchEnd == matchStart) {
      if (matchStart < len) {
        final nextChar = WasmStringImplementation.substring(
          string,
          matchStart,
          matchStart + 1,
        );
        result = WasmStringImplementation.concat(result, nextChar);
      }
      start = matchStart + 1;
    } else {
      start = matchEnd;
...
WasmStringImplementation wasmStringFromDartString(String s) {
  final len = s.length;
  bool isLatin1 = true;
  for (int i = 0; i < len; i++) {
    if (s.codeUnitAt(i) > 0xFF) {
      isLatin1 = false;
      break;
    }
  }
  if (isLatin1) {
    final arr = WasmArray<WasmI8>(len);
    for (int i = 0; i < len; i++) {
      arr.write(i, s.codeUnitAt(i));
    }
    return Latin1String.unsafeWrap(arr);
  } else {
    final arr = WasmArray<WasmI16>(len);
    for (int i = 0; i < len; i++) {
      arr.write(i, s.codeUnitAt(i));
    }
    return Utf16String.unsafeWrap(arr);
  }
}
